...
def proceso_detalle_view() -> rx.Component:
    """Vista de detalle: Barridos y ofertas de un proceso específico"""
    return rx.vstack(
        # Breadcrumb
        rx.hstack(
            rx.link(
        ...
        rx.card(
            rx.vstack(
                rx.heading("Detalle del Proceso", size="7"),
                rx.hstack(
                    rx.text("Código:", weight="bold"),
                    rx.text(ProcesosState.proceso_actual.codigo_proceso if ProcesosState.proceso_actual else "-", font_family="monospace"),
                    spacing="2"
                ),
                rx.hstack(
                    rx.text("Nombre:", weight="bold"),
                    rx.text(ProcesosState.proceso_actual.nombre if ProcesosState.proceso_actual else "-"),
                    spacing="2"
                ),
                spacing="3",
                align_items="start"
                    ...
                    lambda b: rx.table.row(
                        ...
                        rx.table.cell(
                            rx.button(
                                ...
                                variant="soft",
                                color_scheme=rx.cond(
                                    ProcesosState.barrido_seleccionado_id == b.id,
                                    "grass",
                                    "gray"
                                )
                            )
